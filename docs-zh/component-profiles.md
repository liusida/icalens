...
选择负尾。结果保存为 `tail_direction`，并为兼容性同步保存到 `dominant_sign`。偏度
恰好为零时，回退到分数平方能量较大的一侧。

偏度是标准化三阶中心矩，用来衡量分数分布相对于均值的不对称性。所选尾部只在画像
标题中显示一次。Logit Lens 和 R-lens 旁边的紧凑统计栏显示偏度、超额峰度和拟合时的
logcosh 偏差。后者是相对于高斯对比基线的绝对距离，用于排列成分 ID；其排名可能与
超额峰度排名不同。完整的符号频率与平方能量比例仍保存在画像数据中。
